...
trait DBHandling {
  ...
  private def latestCommon[A, B](as: Seq[A], bs: Seq[B])(f: A => Seq[Byte])(g: B => Seq[Byte]): Option[(A, B)] = {
    val ahs = as.map(a => (a, f(a)))
    val bhs = bs.map(b => (b, g(b)))
    (ahs zip bhs).takeWhile(ahbh => (ahbh._1._2 == ahbh._2._2)).lastOption.map(ahbh => (ahbh._1._1, ahbh._2._1))
  }
  ...
  private def applyMigration(t: Transaction, m: Migration, hash: Seq[Byte]): Either[String, Transaction] = {
